...
import { 
  ...
  CheckCircle2, ChevronRight, X, BarChart3, MessageSquare, Target
} from 'lucide-react';
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-[rgb(var(--sx-modal))] border border-[rgb(var(--sx-border))] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-100">
        
        {/* Search Input Bar */}
        <div className="flex items-center px-4 py-3 border-b border-[rgb(var(--sx-border))]">
              ...
              <div className="space-y-1">
                {filteredTasks.map(t => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs text-left hover:bg-[rgb(var(--sx-chip))] text-stone-200 group transition-colors"
                  >
                    <div className="flex items-center space-x-2 truncate">
                      <CheckCircle2 size={14} className={t.status === 'done' ? 'text-emerald-400' : 'text-stone-400'} />
                      <span className="truncate">{t.title}</span>
                    </div>
                    <span className="text-[10px] text-stone-400 font-mono">{t.category}</span>
                  </button>
                ))}
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-2.5">
                      <Icon size={15} className="text-amber-400 group-hover:scale-110 transition-transform" />
                      <span>{act.label}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      ...
                      <ChevronRight size={13} className="text-stone-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                  </button>
                );
              })}
        ...
        <div className="px-4 py-2 border-t border-[rgb(var(--sx-border)/0.6)] text-[10px] text-stone-400 flex items-center justify-between">
