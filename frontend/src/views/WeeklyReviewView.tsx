...
import { BarChart3, CheckCircle2, AlertCircle, Repeat, TrendingUp, Loader2, Sparkles, Calendar } from 'lucide-react';
    ...
    <div className="max-w-full space-y-5 animate-in fade-in duration-200">
      ...
      {loading ? (
        <div className="py-16 flex justify-center items-center space-x-2">
          ...
          <span className="text-sm text-slate-400">Analyzing your week...</span>
        </div>
      ) : error ? (
        <div className="py-12 text-center text-slate-400">
          <AlertCircle size={32} className="mx-auto mb-3 text-amber-400" />
          <p>{error}</p>
          <button onClick={fetchReview} className="mt-3 text-sm text-indigo-400 hover:text-indigo-300">Try Again</button>
        </div>
      ) : review ? (
        <div className="space-y-5">
          {/* Summary Card */}
          <div className="p-6 rounded-xl bg-[#12141c] border border-[#262a3c] space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-slate-200">Summary</h2>
            ...
            {/* Progress bar */}
            <div className="w-full bg-[#1e2230] rounded-full h-3 overflow-hidden">
              <div
          ...
          {review.highlights.length > 0 && (
            <div className="p-5 rounded-xl bg-[#12141c] border border-emerald-500/20 space-y-3">
          ...
          {review.suggestions.length > 0 && (
            <div className="p-5 rounded-xl bg-[#12141c] border border-indigo-500/20 space-y-3">
