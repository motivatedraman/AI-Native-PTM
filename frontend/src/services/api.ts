...
export const api = {
  ...
  async reopenTask(id: number): Promise<Task> {
    const res = await fetch(`${API_BASE}/tasks/${id}/reopen`, { method: 'POST' });
  ...
  async enrichTaskWithAI(taskId: number) {
    const res = await fetch(`${API_BASE}/ai/enrich-task/${taskId}`, { method: 'POST' });
  ...
  async suggestSubtasksWithAI(taskId: number): Promise<{ task_id: number; suggested_subtasks: string[] }> {
    const res = await fetch(`${API_BASE}/ai/suggest-subtasks/${taskId}`, { method: 'POST' });
